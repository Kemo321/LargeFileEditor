...
void LineManager::invalidateCacheFromOffset(uint64_t offset) {
    std::lock_guard<std::mutex> lock(cache_mutex_);
    if (line_start_offsets_.empty()) return;
...
void LineManager::ensureOffsetCalculated(uint64_t target_offset) {
    ...
    while (current_offset < target_offset && current_offset < file_size) {
        ...
        while (!line_ended && search_offset < file_size) {
            ...
            uint64_t max_fetch = std::min(to_fetch, static_cast<uint64_t>(max_visual_line_length_) - (search_offset - line_start));
            ...
            if (newline_pos != std::string::npos) {
                ...
                if (search_offset >= file_size) {
                    ...
                    line_ended = true;
                } else if (search_offset - line_start >= static_cast<uint64_t>(max_visual_line_length_)) {
                    current_offset = search_offset;
                    line_ended = true;
                }
            }
...
int LineManager::getLineCount() {
    ...
    std::lock_guard<std::mutex> lock(cache_mutex_);
    if (line_start_offsets_.empty()) return 1;
    ...
    int estimated_remaining = static_cast<int>((file_size - processed_bytes) / avg_bytes_per_line);
